...
describe('ReimbursementsPage', () => {
  ...
  it('offers to add a missing category while cataloguing a purchase', async () => {
    const data = structuredClone(defaultData)
    const purchase = {
      id: 'ordinary-purchase', payerId: users[1].id, recipientId: users[0].id,
      payerMovementId: 'payer-movement', amount: 35, purchaseDate: '2026-08-29',
      description: 'Cena', status: 'pending' as const, createdAt: '2026-08-29T10:00:00Z',
    }
    const onRespondPurchase = vi.fn().mockResolvedValue(undefined)
    ...
    await waitFor(() => expect(onRespondPurchase).toHaveBeenCalledWith(
      purchase, true, expect.any(String), expect.any(String), expect.objectContaining({ name: 'Cene occasionali' }),
    ))
  })
