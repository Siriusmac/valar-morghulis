...
export function MovementsPage({ data, user, onEdit, onDelete, onEditTransfer, onDeleteTransfer }: Props) {
  ...
  return <div className="page movements-page">
    <div className="page-heading"><div><h1>Spese ed Entrate</h1><p>Analizza spese, entrate e giro fondi.</p></div></div>
    <div className="movement-toolbar">
      <div className="tabs movement-tabs"><button className={section === 'expense' ? 'active' : ''} onClick={() => setSection('expense')}>Spese</button><button className={section === 'income' ? 'active tab-income' : 'tab-income'} onClick={() => setSection('income')}>Entrate</button><button className={section === 'shared' ? 'active' : ''} onClick={() => setSection('shared')}>Condivise</button><button className={section === 'transfer' ? 'active tab-transfer' : 'tab-transfer'} onClick={() => setSection('transfer')}>Giro fondi</button></div>
    ...
    <section className="movements-ledger"><div className="section-title-row"><div><h2>Movimenti</h2><p>{resultCount} {resultCount === 1 ? 'risultato' : 'risultati'} nel mese{section === 'shared' ? ' · importi condivisi' : ''}</p></div><label className="search-field"><Search /><input placeholder={section === 'transfer' ? 'Cerca giro fondi o conto' : 'Cerca movimento o tag'} value={query} onChange={(e) => setQuery(e.target.value)} /></label></div>{section === 'transfer' ? <TransferList data={data} transfers={transferList} user={user} onEdit={onEditTransfer} onDelete={onDeleteTransfer} /> : <MovementList data={data} movements={list} user={user} onEdit={onEdit} onDelete={onDelete} sharedAmountsOnly={section === 'shared'} />}</section>
  </div>
}

function TransferList({ data, transfers, user, onEdit, onDelete }: { data: AppData; transfers: Transfer[]; user: User; onEdit: (transfer: Transfer) => void; onDelete: (id: string) => void }) {
  ...
  return <div className="movement-list transfer-list">{transfers.map((transfer) => {
    ...
    const canEdit = transfer.authorId === user.id
