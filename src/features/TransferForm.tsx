...
export function TransferForm({ data, user, memberCount = 2, onSubmit, onCancel }: { data: AppData; user: User; memberCount?: number; onSubmit: (transfer: Transfer) => void | Promise<void>; onCancel: () => void }) {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    setBusy(true)
    try {
      await onSubmit({ id: makeId('transfer'), authorId: user.id, fromAccountId, toAccountId, amount: value, date, description: description.trim() || 'Giro fondi' })
    } catch (reason) {
