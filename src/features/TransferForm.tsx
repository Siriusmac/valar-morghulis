...
export function TransferForm({ data, user, memberCount = 2, initial, onSubmit, onDelete, onCancel, onRequireBankInstitution }: Props) {
  ...
  const [feeAmount, setFeeAmount] = useState(() => initial?.feeAmount ? initial.feeAmount.toFixed(2).replace('.', ',') : '')
  ...
  const destinationAccounts = accounts.filter((item) => item.id !== fromAccountId)
  const supportsTransferFees = Boolean(fromAccount && fromAccount.type !== 'cash' && fromAccount.type !== 'welfare')
  const createsFamilyDebt = fromAccount?.scope === 'family' && toAccount?.scope === 'personal'
  const submit = async (event: React.FormEvent) => {
    event.preventDefault()
    const value = Number(amount.replace(',', '.'))
    const feeValue = supportsTransferFees && feeAmount.trim() ? Number(feeAmount.replace(',', '.')) : 0
    ...
    if (!Number.isFinite(feeValue) || feeValue < 0) { setError('Le spese bancarie non possono essere negative.'); return }
    setError('')
    setBusy(true)
    try {
  ...
  if (accounts.length < 2) return <div className="empty-state transfer-empty"><ArrowLeftRight /><h3>Servono almeno due conti</h3><p>Crea un altro conto prima di registrare un giro fondi.</p><button type="button" className="button button--ghost" onClick={onCancel}>Chiudi</button></div>

  return <form className="transfer-form composer-fields composer-fields--enter" onSubmit={submit}><span className="reimbursement-form__icon"><ArrowLeftRight /></span><label>Dal conto<select value={fromAccountId} disabled={busy} onChange={(e) => { const nextFrom = e.target.value; setFrom(nextFrom); if (toAccountId === nextFrom) setTo(accounts.find((item) => item.id !== nextFrom)?.id ?? ''); setFeeAmount(''); setError('') }}>{accounts.map((item) => <option key={item.id} value={item.id}>{item.name} · {formatMoney(accountBalance(data, item.id))}</option>)}</select></label><label>Al conto{destinationAccounts.length === 1 ? <output>{destinationAccounts[0].name}</output> : <select value={toAccountId} disabled={busy} onChange={(e) => { setTo(e.target.value); setError('') }}>{destinationAccounts.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select>}</label>{createsFamilyDebt ? <p className="transfer-note">Il conto di origine è condiviso: il {otherMembersPercentage} dell’importo verrà contabilizzato come debito del titolare del conto personale verso gli altri membri.</p> : null}<label>Importo trasferito<div className="money-input"><span>€</span><input aria-label="Importo" inputMode="decimal" value={amount} disabled={busy} onChange={(e) => { setAmount(e.target.value); setError('') }} autoFocus /></div></label>{supportsTransferFees && fromAccount ? <label>Spese bancarie<div className="money-input"><span>€</span><input aria-label="Spese bancarie" inputMode="decimal" value={feeAmount} disabled={busy} placeholder="0,00" onChange={(e) => { setFeeAmount(e.target.value); setError('') }} /></div><small className="field-help">Vengono addebitate al conto di origine e assegnate a “Commissioni {fromAccount.institution.trim() || fromAccount.name}”.</small></label> : null}<label>Descrizione<input value={description} disabled={busy} onChange={(e) => setDescription(e.target.value)} /></label><label>Data<input type="date" value={date} disabled={busy} onChange={(e) => setDate(e.target.value)} /></label>{error ? <p className="form-message form-message--error" role="alert">{error}</p> : null}<div className={`form-actions ${initial ? 'form-actions--edit' : ''}`}>{initial && onDelete ? <button className="button button--danger form-actions__delete" type="button" disabled={busy} onClick={() => confirm('Eliminare questo giro fondi? I saldi dei conti verranno aggiornati.') && void onDelete(initial.id)}><Trash2 />Elimina giro fondi</button> : null}<button type="button" className="button button--ghost" disabled={busy} onClick={onCancel}>Annulla</button><button className="button button--primary" type="submit" disabled={busy}>{busy ? 'Salvataggio…' : initial ? 'Salva modifiche' : 'Conferma giro fondi'}</button></div></form>
}
